...
//   - PriorityQueue：优先级队列，按优先级高低排序消费（不支持幂等配置，构造时 panic 拒绝）
...
//   - BoundedBlockingQueue：有界阻塞队列，固定容量、满时阻塞生产者、空时阻塞消费者（不支持幂等配置，构造时 panic 拒绝）
//   - TimerQueue：定时队列，按绝对时间点或相对时长调度入队
//
// 核心设计理念：
//
//   - 幂等语义：开启 WithValueIdempotent 后，相同值不会被重复入队。
//     支持范围为复用基础队列幂等簿记的队列（Queue/RetryQueue/LeasedQueue/
//     DeadLetterQueue/TimerQueue 等）；BoundedBlockingQueue 与 PriorityQueue
//     因结构不兼容（信号量令牌配对/堆序无法与挂起标记、Done 重入队协同），
//     构造时 panic 拒绝该配置（fail-fast）
